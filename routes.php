    ...
    <main id="container">
        ...
        <?php 
            ...
            {?>
                <div class="container mt-4">
                    <div id="noRoutes" class="alert alert-dark " role="alert">
                        ...
                        <p class="fw-light">
                            Hiện tại không có dịch vụ nào cho tuyến đường được chỉ định
                        </p>
                        <hr>
                        <div id="addCustomerAlert" class="alert alert-success" role="alert">
                            Chúng tôi sẽ sớm cung cấp tuyến đường này cho dịch vụ
                        </div>
                    </div>
                </div>
            <?php }
            while($row = mysqli_fetch_assoc($result))
            {
                ...
                // Tìm kiếm trong bảng nếu chúng tôi có bất kỳ tuyến đường nào phù hợp với chi tiết biểu mẫu
                if(!in_array($source, $citiesArr) || !in_array($destination, $citiesArr) || !(array_search($source, $citiesArr) < array_search($destination, $citiesArr)))
                    continue;
